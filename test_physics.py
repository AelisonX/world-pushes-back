...
def test_higher_static_friction_raises_slip_threshold():
    ...
    low_threshold = slip_transition_force(
        low_friction,
        angle_deg=45.0,
    )

    high_threshold = slip_transition_force(
        high_friction,
        angle_deg=45.0,
    )
...
def test_heavier_container_raises_slip_threshold():
    ...
    light_threshold = slip_transition_force(
        light,
        angle_deg=45.0,
    )

    heavy_threshold = slip_transition_force(
        heavy,
        angle_deg=45.0,
    )

    assert heavy_threshold > light_threshold
...
def test_stronger_material_raises_yield_threshold():
    ...
    weak_threshold = material_transition_force(
        weak,
        angle_deg=45.0,
    )

    strong_threshold = material_transition_force(
        strong,
        angle_deg=45.0,
    )

    assert strong_threshold > weak_threshold
...
def test_larger_contact_area_raises_yield_threshold():
    ...
    small_threshold = material_transition_force(
        small_area,
        angle_deg=45.0,
    )

    large_threshold = material_transition_force(
        large_area,
        angle_deg=45.0,
    )
...
def test_compliant_support_motion_increases_near_slip_threshold():
    far_from_slip = compute_pre_slip_support_motion(
        true_fx=2.0,
        slip_force=20.0,
        pre_slip_displacement_limit=0.002,
    )

    near_slip = compute_pre_slip_support_motion(
        true_fx=18.0,
        slip_force=20.0,
        pre_slip_displacement_limit=0.002,
    )

    assert abs(near_slip) > abs(far_from_slip)


def test_pre_slip_support_motion_is_bounded():
    displacement = compute_pre_slip_support_motion(
        true_fx=50.0,
        slip_force=20.0,
        pre_slip_displacement_limit=0.002,
    )

    assert abs(displacement) <= 0.002